...
class BrandApp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  async addBrand() {
    ...
    const res_add = await API_CONNECT(
      ConstantApp.ADD_BRAND, body, "", "POST")
  ...
  async updateBrand() {
    ...
    const res = await API_CONNECT(
      ConstantApp.UPDATE_BRAND, body, "", "POST")
  ...
  render() {
    const { data, arrPagination, key,name, image } = this.state;
