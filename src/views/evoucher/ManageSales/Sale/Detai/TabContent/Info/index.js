...
function InfoProvider({ infoCustomer }) {

    React.useEffect(() => {
        console.log("infoCustomer", infoCustomer);
    }, [infoCustomer]);
  const addressInfo = infoCustomer?.addressInfo;
  const email = infoCustomer?.email;
  const phone = infoCustomer?.phone;
  const website = infoCustomer?.website;
  const name = infoCustomer?.name;
  const username = infoCustomer?.username;
  const partnerID = infoCustomer?.partnerID;
    ...
    <div className="tabContentItem" style={styles.tabContentItem}>
      <div className="tabInfo-body">
        <div className="tabInfo-body__contact">
          ...
          <h5 className="tabInfo-body__contact-title mt-3">Thông tin tài khoản</h5>
          <ul className="tabInfo-body__contact-list">
            <li className="tabInfo-body__contact-item">
              Tên
              <p className="tabInfo-body__contact-item-content">
